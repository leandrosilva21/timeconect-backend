...
> **dentro do Minutor** (na conversa do fechamento), não só no Outlook.

## Como funciona (já implementado no Minutor)
...
- Um job do Minutor (`fechamento:poll-inbox`, a cada 5 min) **lê** essa caixa via **Microsoft Graph**, filtra **só** as respostas de fechamento (casando `In-Reply-To`/`References` com o `Message-ID` que o Minutor gerou) e grava na conversa. As demais mensagens do `noreply` (outros workflows) são **ignoradas**.
- **Sem credenciais do Graph, o job dorme** (não quebra nada). Ele "acende" assim que o TI entregar os 3 valores abaixo.

## O que o Minutor precisa do TI (uma vez)

Registrar um **app no Azure AD (Entra ID)** com permissão de **aplicação** para ler **apenas** a caixa do `noreply`:

1. **Azure Portal → Entra ID → App registrations → New registration**
   - Nome sugerido: `Minutor — Leitura Fechamento (noreply)`
...
4. **(Recomendado — escopo mínimo) Application Access Policy** no Exchange Online, pra esse app **só** conseguir ler a caixa do `noreply` (e nenhuma outra). Via Exchange Online PowerShell:
   ...
   New-DistributionGroup -Name "Minutor-Graph-Scope" -Type Security -Members noreply@erpserv.com.br

   # restringe o app (use o Application/Client ID do passo 1) a esse grupo
   New-ApplicationAccessPolicy -AppId <CLIENT_ID> `
     -PolicyScopeGroupId Minutor-Graph-Scope@erpserv.com.br `
     -AccessRight RestrictAccess `
     -Description "Minutor só lê a caixa do noreply (fechamento)"
...
## Ativação (lado Minutor, depois que o TI entregar)
...
3. Teste manual: `docker exec minutor-backend php artisan fechamento:poll-inbox`
...
- O Minutor só persiste respostas que casam com uma thread de fechamento; o resto do `noreply` é ignorado e não é gravado.
